...
test("server VPN Overall is restricted to internal node projects", async () => {
  ...
  const hourlyRankIndex = service.indexOf("ROW_NUMBER() OVER (PARTITION BY project_code, app_identifier, connection_id");
  const latestNodeEligibilityIndex = service.indexOf("NULLIF(TRIM(CAST(normalized_server_id AS CHAR)), '') IS NOT NULL", hourlyRankIndex);
  assert.ok(hourlyRankIndex >= 0 && latestNodeEligibilityIndex > hourlyRankIndex, "latest explicit result must be ranked before blank-node exclusion");
