...
export function ActionDialog({ dialog, project, configs = trackingConfigs, editingConfig = null, onClose, onSubmit }: { dialog: DialogKey; project: string; configs?: TrackingConfigRecord[]; editingConfig?: TrackingConfigRecord | null; onClose: () => void; onSubmit: (result: DialogResult) => void }) {
  ...
  const dictionaryRows = useMemo(() => [
    ["jk_ad_request", "广告请求", "P0", "13", "稳定可得", "V1.8"],
    ["jk_ad_impression", "广告展示", "P0", "11", "广告SDK回调", "V1.8"],
    ["connect_success", "VPN连接", "P0", "10", "业务模块", "V1.8"],
    ["app_background", "生命周期", "P1", "8", "推断可得", "V1.6"],
    ["jk_ad_paid_event", "收入回调", "条件P0", "9", "广告SDK回调", "V1.8"],
  ], []);
          ...
          <form ref={formRef} onSubmit={submit} noValidate={false}>
            <div className="modal-body">
              ...
              {(dialog === "project-report" || dialog === "admob-report") && <div className="modal-form-grid">
                ...
                <Field label="口径版本" required><select name="metric_version" required><option value="V1.8">V1.8 当前执行权威</option><option value="V1.7">V1.7</option></select></Field>
              ...
              {dialog === "version-diff-report" && <div className="modal-form-grid">
                ...
                <Field label="目标版本" required><select name="target_version_id" required><option value="cfg_v18">V1.8（当前执行权威）</option></select></Field>
              ...
              {dialog === "reconcile-run" && <div className="modal-form-grid">
                ...
                <Field label="口径版本" required><select name="metric_version" required><option>V1.8</option><option>V1.7</option></select></Field>
              ...
              {dialog === "config-version" && <>
                <div className={`modal-form-grid config-step-panel ${configStep === 1 ? "" : "config-step-hidden"}`}>
                  ...
                  <Field label="基础配置"><select name="base_config_id" defaultValue={editingConfig?.id ?? "CFG-VPN-1.8-PROD"}><option value={editingConfig?.id ?? "CFG-VPN-1.8-PROD"}>{editingConfig ? `${editingConfig.name} ${editingConfig.version}` : `VPN 正式版 V1.8（复制${defaultSelectedEventIds.length}项）`}</option><option value="blank">空白配置</option></select></Field>
                  ...
                  <Field label="配置说明" required span><textarea name="description" defaultValue={`从全量${trackingEventCatalog.length}个事件中选择本版本必须实现并验收的${editingConfig?.selectedCount ?? defaultSelectedEventIds.length}个事件。`} required /></Field>
                </div>
                ...
                <div className={`config-rule-review config-step-panel ${configStep === 3 ? "" : "config-step-hidden"}`}>
                  <div className="rule-review-grid"><div><span>本配置事件</span><strong>{selectedConfigEventIds.length}/{trackingEventCatalog.length}</strong><small>只验收已选事件</small></div><div><span>P0 / P1 / P2</span><strong>{selectedPriorityCounts.P0} / {selectedPriorityCounts.P1} / {selectedPriorityCounts.P2}</strong><small>可逐事件覆盖默认级别</small></div><div><span>业务场景</span><strong>12</strong><small>场景必须先标记执行</small></div><div><span>关联链</span><strong>8 条</strong><small>广告、VPN、订阅主链</small></div></div>
                ...
                <div className={`config-validation config-step-panel ${configStep === 4 ? "" : "config-step-hidden"}`}>
                  ...
                  <div className="validation-list"><div><span>✓</span><p><strong>已选择 {selectedConfigEventIds.length}/{trackingEventCatalog.length} 个事件</strong><small>未选事件不会进入本配置的验收分母</small></p></div><div><span>✓</span><p><strong>{selectedPriorityCounts.P0} 个 P0 事件均有必填参数规则</strong><small>P0事件、P0参数、P0关联链必须100%</small></p></div><div><span>✓</span><p><strong>12 个场景已绑定事件</strong><small>场景未执行与事件未收到分开统计</small></p></div><div><span>✓</span><p><strong>发布后生成不可变 snapshot_id</strong><small>历史Run不受后续配置修改影响</small></p></div></div>
                  <Field label="发布备注" required span><textarea name="release_note" defaultValue={`完成${selectedConfigEventIds.length}个必测事件选择与规则校验，发布后供V1.8客户端验收使用。`} required /></Field>
              ...
              {dialog === "firebase-connection" && <div className="modal-form-grid">
                ...
                <Field label="认证方式" required><select name="auth_type" value={authType} onChange={(event) => setAuthType(event.target.value)} required><option value="SERVICE_ACCOUNT">服务账号</option><option value="WORKLOAD_IDENTITY">Workload Identity</option></select></Field>
                ...
                <Field label={authType === "WORKLOAD_IDENTITY" ? "身份配置引用" : "凭证密钥引用"} required span help="只保存密钥管理系统引用，禁止在系统数据库保存JSON明文"><input name="credential_secret_ref" placeholder={authType === "WORKLOAD_IDENTITY" ? "secret://firebase/workload-identity/growth" : "secret://firebase/jkcl-growth-prod"} pattern="^secret://.+" required /></Field>
                <Field label="服务账号邮箱" required span help="只用于权限核对与审计，不是密钥"><input type="email" name="service_account_email" placeholder="firebase-reader@example.iam.gserviceaccount.com" required /></Field>
