...
type VpnReportSectionKey = "matrix" | "serverVpn" | "exitIp" | "adOverall" | "adDns" | "vpnVersions" | "launcherVersions";
const diagnosticReportMenus: Array<{ key: VpnReportSectionKey; label: string; hint: string }> = [
  ...
  { key: "serverVpn", label: "服务器VPN Overall", hint: "内部节点运行质量" },
  ...
  { key: "launcherVersions", label: "Launcher Overall", hint: "Launcher获取、引导、激活与网络维度" },
];
...
const moduleCopy: Record<ModuleKey, { title: string; description: string; action: string }> = {
  ...
  vpnReport: { title: "诊断报表", description: "包含 VPN Overall、服务器VPN Overall、出口IP质量、广告漏斗Overall、广告DNS诊断、VPN版本对比和Launcher Overall；支持查看内部节点与关联Session证据", action: "导出诊断报表" },
...
export default function Home() {
  ...
  const sourceStatus: Record<ModuleKey, { title: string; detail: string; note: string }> = {
    ...
    vpnReport: { title: "诊断报表", detail: "VPN Overall · VPN版本对比 · Launcher Overall", note: "VPN版本使用连接、探测和广告打点；Launcher Overall仅允许L开头项目，并展示获取、引导、激活及已上报网络维度。" },
      ...
      <div className="workspace">
        ...
        <main className="main-content" style={{ display: operationsActive ? "none" : undefined }}>
          ...
          {module === "funnel" && <OperationalFunnel
            ...
            onPageChange={(nextPage) => go(nextPage as PageKey)}
