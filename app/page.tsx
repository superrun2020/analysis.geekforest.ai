      ...
      <div className="workspace">
        ...
        <main className="main-content">
          <section className="page-heading">
            ...
            <div className="heading-actions"><button className="secondary-button" onClick={() => go("snapshot")}>查看口径 V1.7</button><button className="primary-button" onClick={() => go("issues")}>＋ 新建诊断任务</button></div>
          </section>
            ...
            <div className="page-stack">
              <section className="analysis-head surface">
                ...
                <div className="mode-controls">
                  <Segmented label="漏斗类型" active={funnelMode} onChange={(key) => setFunnelMode(key as FunnelMode)} items={[{ key: "product", label: "产品漏斗" }, { key: "monetization", label: "变现漏斗" }]} />
                  <Segmented label="统计单位" active={unitMode} onChange={(key) => setUnitMode(key as UnitMode)} items={[{ key: "users", label: "用户数" }, { key: "events", label: "事件数" }]} />
                </div>
              </section>
            ...
            <div className="page-stack">
              ...
              <section className="two-column wide-left">
                ...
                <aside className="surface">
                  ...
                  <div className="action-stack"><button className="primary-button" onClick={() => go("cohort")}>进入分群对比</button><button className="secondary-button" onClick={() => go("evidence")}>查看事件证据</button><button className="secondary-button" onClick={() => go("issues")}>创建问题任务</button></div>
                </aside>
              </section>
            </div>
          )}
            ...
            <div className="page-stack">
              ...
              <section className="two-column wide-left evidence-layout">
                ...
                <aside className="surface event-detail">
                  ...
                  <button className="primary-button full" onClick={() => go("issues")}>将证据加入问题单</button>
                </aside>
              </section>
            </div>
          )}
          ...
          {page === "issues" && (
            <div className="page-stack">
              <section className="surface">
                <div className="surface-title"><div><h2>ISSUE-20260811-024 · Opportunity 覆盖率异常</h2><p>IRAN-VPN-01 · P0 · 负责人：客户端增长组 / Oliver</p></div><Badge tone="warn">修复中</Badge></div>
                <div className="issue-workflow">{[["已发现", "8/11 10:32"], ["已定位", "8/11 11:08"], ["修复中", "预计 8/12"], ["打点重测", "待开始"], ["灰度验证", "待开始"], ["关闭", "指标恢复"]].map(([label, time], index) => <div key={label} className={`${index < 2 ? "done" : index === 2 ? "current" : ""}`}><span>{index < 2 ? "✓" : index + 1}</span><strong>{label}</strong><small>{time}</small></div>)}</div>
              </section>
              <section className="two-column wide-left">
                <div className="surface">
                  ...
                  <div className="action-bar"><button className="secondary-button">保存修复说明</button><button className="primary-button">发起打点重测</button></div>
                </div>
                <aside className="surface">
                  <div className="surface-title"><div><h2>发布门禁</h2><p>修复版本必须全部通过</p></div></div>
                  <div className="gate-list"><div><span>P0事件完整率</span><strong>目标 100%</strong><Badge tone="neutral">待测</Badge></div><div><span>P0参数完整率</span><strong>目标 100%</strong><Badge tone="neutral">待测</Badge></div><div><span>关联链完整率</span><strong>目标 100%</strong><Badge tone="neutral">待测</Badge></div><div><span>Unknown率</span><strong>目标 &lt; 1%</strong><Badge tone="neutral">待测</Badge></div></div>
                  <div className="conclusion-block warn"><strong>当前不可发布</strong><p>完成打点重测且 P0 指标达到 100% 后，才允许进入灰度。</p></div>
                </aside>
              </section>
              <section className="surface">
          ...
          {page === "snapshot" && (
            <div className="page-stack">
              <section className="snapshot-head surface"><div><div className="eyebrow">只读执行快照</div><h2>IRAN-VPN-01 · 漏斗口径 V1.7</h2><p>生效于 2026-08-01 · 套利 VPN v1.7 ＋ 广告 v1.7 ＋ VPN v1.7</p></div><div><Badge tone="good">已发布</Badge> <button className="secondary-button">前往项目类型设置</button></div></section>
