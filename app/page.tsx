          ...
          {module === "funnel" && page === "workbench" && (
            <div className="page-stack">
              ...
              <section className="surface funnel-surface">
                ...
                {funnelMode === "monetization" && unitMode === "users" && <div className="denominator-audit"><div><span>广告浏览者比例</span><strong>广告浏览用户（AV）/ 日活跃用户（DAU）= {activeProfile.viewerTrend.at(-1)}%</strong><small>衡量覆盖用户</small></div><div><span>机会覆盖率</span><strong>广告机会用户（Opportunity UV）/ 日活跃用户（DAU）= {activeProfile.opportunityTrend.at(-1)}%</strong><small>定位请求前问题</small></div><div><span>请求用户覆盖率</span><strong>广告请求用户（Request UV）/ 广告机会用户（Opportunity UV）= {activeProfile.userStages[4].rate}</strong><small>含预加载与实时请求用户</small></div><div><span>人均展示</span><strong>广告展示次数（Impression）/ 广告浏览用户（AV）= 3.42</strong><small>衡量展示集中度</small></div></div>}
