...
export default function SharedReportPage() {
  ...
  async function loadMeta(nextShareId: string) {
    setLoading(true);
    setMessage("");
    try {
      const next = await publicApi<ShareMeta>(`/api/v3/jkcl-funnel/shared-reports/${encodeURIComponent(nextShareId)}/meta`);
      setMeta(next);
  ...
  async function openReport(nextPassword = password) {
    if (!shareId) {
      setMessage("链接缺少报告 ID，请重新复制分享链接。");
      return;
    }
  ...
  useEffect(() => {
    if (typeof window === "undefined") return;
    const params = new URLSearchParams(window.location.search);
    const sid = params.get("sid") || params.get("shareId") || "";
    setShareId(sid);
    if (!sid) {
      setLoading(false);
      setMessage("链接缺少报告 ID，请重新复制分享链接。");
  ...
  useEffect(() => {
    if (autoOpened.current || loading || !shareId || !accessTokenFromUrl) return;
    autoOpened.current = true;
    setPassword(accessTokenFromUrl);
    void openReport(accessTokenFromUrl);
  }, [loading, shareId, accessTokenFromUrl]);
